...
def wifi_vuln_report(report_dir, ssid, bssid, target_info, vuln_info, rogue_info, crack_result=None, all_networks_df=None):
    ...
    filename = os.path.join(
        report_dir, f"wifi_full_{safe_bssid}_{timestamp}.txt")
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        if all_networks_df is not None and not all_networks_df.empty:
            f.write(tabulate(all_networks_df, headers="keys",
        ...
        f.write("-" * 80 + "\n")
        f.write(remove_emojis(rogue_info.get(
            "detailed_log", "No detailed rogue log available.")))
        f.write("\n\n")
        ...
        if crack_result and isinstance(crack_result, dict):
            f.write(
                f"Cracked Password   : {crack_result.get('password', 'N/A')}\n")
            analysis = crack_result.get("analysis", {})

            if analysis.get("strength_issues"):
                f.write("Strength Issues    : " +
                        ", ".join(analysis["strength_issues"]) + "\n")
            else:
                f.write("Password Strength  : Good\n")

            if analysis.get("pwned_count"):
                f.write(
                    f"Leaked in Breaches : {analysis['pwned_count']} times\n")
