        ...
        for chunk in stream:
            ...
            if token.strip():
                ...
                try:
                    ...
                    break  # Client reset connection, stop streaming
                except Exception as e:
                    print(f"[!] Error sending data to client: {e}")
                    break

        # Only cache if we completed the full generation
        if len(response) > 0:
            complete_response = ''.join(response)
            cache.set(prompt, complete_response)
            print(f"[+] Generated in {time.time()-start:.2f}s")

    except BrokenPipeError:
        print(f"[!] Connection broken by client: {addr}")
    except ConnectionResetError:
        print(f"[!] Connection reset by client: {addr}")
    except json.JSONDecodeError:
        print(f"[!] Invalid JSON from client: {addr}")
    except Exception as e:
        print(f"[!] Error: {e}")
        try:
